...
public class JPSPlus
{
    ...
    private readonly SimplePriorityQueue<(JPSPlusNode Node, EDirFlags Dir)> mOpenList = new SimplePriorityQueue<(JPSPlusNode Node, EDirFlags Dir)>();
    ...
    public bool Step(int stepCount)
    {
        ...
        while (true)
        {
            ...
            for (int i = 0b10000000; i > 0; i >>= 1)
            {
                ...
                else if (mOpenList.Contains(openJump) && nextG < nextNode.G)
                {
                    nextNode.Parent = currNode;
                    nextNode.G = nextG;
                    nextNode.H = H(nextNode, mGoal);
    ...
    public LinkedList<GridTile> GetGridPath()
    {
        LinkedList<GridTile> path = new LinkedList<GridTile>();
        AStarNode n = mGoal;
        while (n != null)
        {
            GridTile g = new GridTile(n.Position.X, n.Position.Y);
            path.AddFirst(g);
            n = n.Parent;
        }
        return path;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public SimplePriorityQueue<(JPSPlusNode, EDirFlags)> GetOpenList()
    {
        return mOpenList;
    }
