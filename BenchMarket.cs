...
class Benchmark
{
    ...
    public static List<TestCase> ReadTestCases(FileInfo f)
    {
    ...
    public static List<TestCase> ReadNewTestCases(FileInfo f)
    {
        List<TestCase> tests = new List<TestCase>();
        TestCase test;
        string line;
...
class TestResult
{
    ...
    public PathfindResult RegionResult;
}
...
class PathfindResult
{
    ...
    public void printPath(LinkedList<Edge> currentPath)
    {
        LinkedListNode<Edge> current = currentPath.First;
        while (current != null) {
            if (current.Value.UnderlyingPath == null)
            {
                Console.WriteLine("start:" + current.Value.start.getPos() + ",end:" + current.Value.end.getPos() + ",weight:" + current.Value.weight);
            }            
            else
            {
                printPath(current.Value.UnderlyingPath);
            }
            current = current.Next;
        }
